"""Stage 1: enumerate a source dataset's audio files into sharded paths parquet.
    ...
    python scan_paths.py --scanner dir --audio-root /data/xxx --out paths_dir/
    # driven by stage-0 top entries (progress + resumable per top entry)
    python scan_paths.py --scanner top --audio-root /data/xxx --top-dir top_dir/ --out paths_dir/

Records paths relative to --audio-root plus the derived `format` column; it does
not open files -- duration probing is stage 2 (build_manifest.py). Output is
sharded (paths_part-NNNNN.parquet), one file per --shard-size rows, so stage 2
can build/resume a manifest shard per paths shard.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--scanner", default="dir", choices=["dir", "top"],
                   help="'dir': walk --audio-root directly; 'top': drive from --top-dir shards")
...
def _assert_out_outside_root(audio_root: str, out: str) -> None:
    ...
    inside --audio-root would re-scan the parquet files it just wrote
    (self-pollution). Require them to be disjoint."""
...
def _make_scanner(args: argparse.Namespace):
    if args.scanner == "top":
        return TopDirScanner(args.audio_root, args.top_dir)
    return DirectoryScanner(args.audio_root)
