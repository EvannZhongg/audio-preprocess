...
def load_asr_model(
    ...
    try:
        ...
        # 如果是旧版本，尝试不添加这些参数
        if "multilingual" in str(e) or "hotwords" in str(e):
            try:
                default_asr_options = faster_whisper.transcribe.TranscriptionOptions(
                    **default_asr_options
                )
                ...
                # 如果还是失败，手动处理参数
                print(f"Warning: TranscriptionOptions API changed, attempting fallback...")
                # 移除可能不兼容的参数
                fallback_options = default_asr_options.copy()
