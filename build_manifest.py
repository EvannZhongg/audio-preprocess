...
paths shard.
    ...
    python build_manifest.py --paths paths_dir/ --root /data/xxx \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--root", required=True, help="dataset root the paths are relative to")
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool, \
        ...
        for paths_shard in shards:
            ...
            for rec in pool.map(partial(probe, root=args.root), read_paths(paths_shard)):
                records.append(rec)
                bar.update(1)
